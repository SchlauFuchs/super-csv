...
public class ParseLongTest {
	...
	public void testValidLongs() {
		...
		// positive values
		assertEquals(POSITIVE_VAL, (long)processor.execute(POSITIVE_VAL, ANONYMOUS_CSVCONTEXT));
		assertEquals(POSITIVE_VAL, (long)processorChain.execute(POSITIVE_VAL, ANONYMOUS_CSVCONTEXT));
		
		// negative values
		assertEquals(NEGATIVE_VAL, (long)processor.execute(NEGATIVE_VAL, ANONYMOUS_CSVCONTEXT));
		assertEquals(NEGATIVE_VAL, (long)processorChain.execute(NEGATIVE_VAL, ANONYMOUS_CSVCONTEXT));
	}
	...
	public void testValidLongStrings() {
		// positive values
		assertEquals(POSITIVE_VAL, (long)processor.execute(POSITIVE_STRING, ANONYMOUS_CSVCONTEXT));
		assertEquals(POSITIVE_VAL, (long)processorChain.execute(POSITIVE_STRING, ANONYMOUS_CSVCONTEXT));
		
		// negative values
		assertEquals(NEGATIVE_VAL, (long)processor.execute(NEGATIVE_STRING, ANONYMOUS_CSVCONTEXT));
		assertEquals(NEGATIVE_VAL, (long)processorChain.execute(NEGATIVE_STRING, ANONYMOUS_CSVCONTEXT));
	}
